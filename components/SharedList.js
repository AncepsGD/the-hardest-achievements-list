...
const DevHoverPanelMemo = React.memo(function DevHoverPanelMemo({ devMode, devPanelRef, hoveredIdxRef, visibleListRef, handleEditRef, handleMoveUpRef, handleMoveDownRef, handleDuplicateRef, handleRemoveRef, handleCopyRef }) {
  ...
  const onMoveUp = (e) => { try { e.preventDefault(); e.stopPropagation(); const i = hoveredIdxRef.current; if (i == null) return; const list = (visibleListRef && visibleListRef.current) ? visibleListRef.current : []; const itm = list[i]; if (handleMoveUpRef && handleMoveUpRef.current) { if (itm && itm.id) handleMoveUpRef.current(itm.id); else handleMoveUpRef.current(i); } } catch (err) { } };
  const onMoveDown = (e) => { try { e.preventDefault(); e.stopPropagation(); const i = hoveredIdxRef.current; if (i == null) return; const list = (visibleListRef && visibleListRef.current) ? visibleListRef.current : []; const itm = list[i]; if (handleMoveDownRef && handleMoveDownRef.current) { if (itm && itm.id) handleMoveDownRef.current(itm.id); else handleMoveDownRef.current(i); } } catch (err) { } };
  const onDuplicate = (e) => { try { e.preventDefault(); e.stopPropagation(); const i = hoveredIdxRef.current; if (i == null) return; const list = (visibleListRef && visibleListRef.current) ? visibleListRef.current : []; const itm = list[i]; if (handleDuplicateRef && handleDuplicateRef.current) { if (itm && itm.id) handleDuplicateRef.current(itm.id); else handleDuplicateRef.current(i); } } catch (err) { } };
  const onDelete = (e) => { try { e.preventDefault(); e.stopPropagation(); const i = hoveredIdxRef.current; if (i == null) return; const list = (visibleListRef && visibleListRef.current) ? visibleListRef.current : []; const itm = list[i]; if (handleRemoveRef && handleRemoveRef.current) { if (itm && itm.id) handleRemoveRef.current(itm.id); else handleRemoveRef.current(i); } } catch (err) { } };
