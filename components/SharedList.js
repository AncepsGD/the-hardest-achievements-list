    ...
    fetch(file)
      ...
      .then(data => {
        ...
        useEffect(() => {
          ...
          try {
            // If there's an active search, show search results and mark the
            // prev filter signature as invalid so we recompute when the user
            // clears the search.
            if (debouncedSearch && String(debouncedSearch).trim()) {
              try { setFiltered(_searchResults || []); } catch (e) { }
              try { prevFilterSigRef.current = null; } catch (e) { }
          ...
          try {
            ...
            // If the signature hasn't changed since the last meaningful
            // compute, skip heavy processing (helps with Strict Mode).
